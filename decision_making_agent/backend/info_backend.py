...
llm1 = ChatGroq(groq_api_key=groq_api_key, model_name=model_name)  # Answer model
llm2 = ChatGroq(groq_api_key=groq_api_key, model_name=model_name)  # Evaluation model
...
#        - There can be slight increase and decrease in number of bullet points in each call
    ...
    print(" in info backend and response is")
    print(ai_response)
    return jsonify({
