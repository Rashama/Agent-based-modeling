...
from utils.utils import display_media
...
def display_practice_task(image_path):
    ...
    if not st.session_state.practice_submitted:
        ...
        # If task has been submitted, display additional info
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("I am the Informational AI manager and have compiled the following information on this matter:")
            st.markdown(f"<div class='success-box ai-manager-info'>{ai_manager_info}</div>", unsafe_allow_html=True)

            # Text area for resubmitting idea
            ...
            #st.session_state.resubmit_idea = st.text_area("💡 Resubmit your idea:", key="resubmit_text", height=100)
        ...
        with col2:
            ...
            encoded_image = display_media(image_path,500)

            # Display image
            st.markdown(
                f'<img src="data:image/png;base64,{encoded_image}" class="practice-image" style="max-width: 100%; height: auto; border-radius: 8px;"/>',
                unsafe_allow_html=True,
            )
