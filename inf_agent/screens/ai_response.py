...
def display_ai_response(image_path):
    ...
    with col1:
        ...
        header_text = "🤖🧠 I am the Informational AI manager and have compiled the following information on this matter:"
        st.markdown(
            """
            <style>
        ...
        bullet_points = "".join(
            f"<li>{line.lstrip('-').strip()}</li>" for line in st.session_state.ai_response if line.strip()
        )
